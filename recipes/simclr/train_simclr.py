...
from src.ntxentloss import nt_xentloss
...
from audax import models


def create_model(*, encoder_cls, half_precision, embedding_dim,
                 **kwargs):
    ...
    return SimCLR(model_cls=encoder_cls,
                  embedding_dim=embedding_dim, dtype=model_dtype, **kwargs)
...
def train_and_evaluate(config: ml_collections.ConfigDict,
    ...
    p_spec_aug, spec_aug_rng = training_utilities.get_spec_augment(config, devices)
    if p_spec_aug:
    ...
    if mode == TrainingMode.CONTRASTIVE:
        cost_fn = nt_xentloss(config.model.temperature)
        model = create_model(encoder_cls=model_cls, half_precision=config.half_precision,
